...
RowType = Sequence[int]
GridType = Sequence[RowType]


def get_directions(*,
                   grid: GridType,
                   rows_count: int) -> Sequence[int]:
    for rows in chunks(grid, rows_count):
        vertical_numbers = zip(*rows)
        horizontal_numbers = chain.from_iterable(chunks(row, rows_count)
                                                 for row in rows)
        main_diagonal_numbers = diagonal_numbers(rows)
        minor_diagonal_numbers = diagonal_numbers(reversed(rows))
        yield from chain(horizontal_numbers,
                         vertical_numbers,
                         main_diagonal_numbers,
                         minor_diagonal_numbers)


def diagonal_numbers(rows: Iterable[RowType]
                     ) -> Iterable[RowType]:
    yield from zip(*[row[index:]
...
def largest_grid_product(*,
                         grid: GridType,
                         rows_count: int) -> int:
    directions = get_directions(grid=grid,
                                rows_count=rows_count)
    directions_products = map(multiply, directions)
    return max(directions_products)
...
assert largest_grid_product(grid=grid,
                            rows_count=4) == 70_600_674
